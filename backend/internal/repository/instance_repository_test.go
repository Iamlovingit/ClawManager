...
func TestBuildV2SchedulerInstanceQueryRequiresWorkspaceTypeAndStatuses(t *testing.T) {
	...
	wantArgs := []any{"creating", "running", "gateway", "lite", "openclaw", "hermes", "deepseek-harness", 25}
	if !reflect.DeepEqual(args, wantArgs) {
		t.Fatalf("args = %#v, want %#v", args, wantArgs)
	}
	if got, want := strings.Count(query, "?"), len(args); got != want {
		t.Fatalf("query placeholder count = %d, want %d arguments: %q", got, want, normalized)
	}
}
...
func TestBuildV2SchedulerInstanceQueryDefaultsLimit(t *testing.T) {
	...
	wantArgs := []any{"creating", "gateway", "lite", "openclaw", "hermes", "deepseek-harness", 100}
	if !reflect.DeepEqual(args, wantArgs) {
		t.Fatalf("args = %#v, want %#v", args, wantArgs)
	}
}
